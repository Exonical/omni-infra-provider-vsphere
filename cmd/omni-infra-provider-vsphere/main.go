...
var rootCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		// Infra provider
		ip, err := infra.NewProvider[resources.Machine](meta.ProviderID, provisioner, infra.ProviderConfig{
...
//
//nolint:gocognit,gocyclo,cyclop,maintidx // building dynamic schema involves multiple inventory queries; acceptable for now.
func buildDynamicSchema(ctx context.Context, vc *govmomi.Client, logger *zap.Logger) (string, error) {
	...
	if dcs, err := f.DatacenterList(ctx, "*"); err == nil {
		...
		for _, dc := range dcs {
			...
			if cls, qErr := f.ClusterComputeResourceList(ctx, "*"); qErr == nil {
				for _, c := range cls {
	...
	if dcs, err := f.DatacenterList(ctx, "*"); err == nil {
		...
		for _, dc := range dcs {
			...
			if dss, dsErr := f.DatastoreList(ctx, "*"); dsErr == nil {
				for _, ds := range dss {
					allDatastores = append(allDatastores, ds.Name())
				}
			} else {
				logger.Warn("failed to query datastores for datacenter", zap.String("datacenter", dc.Name()), zap.Error(dsErr))
			}
		}
		// Reset finder to no specific datacenter
		f.SetDatacenter(nil)
	...
	if dcs, err := f.DatacenterList(ctx, "*"); err == nil {
		...
		for _, dc := range dcs {
			...
			if nets, netErr := f.NetworkList(ctx, "*"); netErr == nil {
				...
				logger.Warn("failed to query networks for datacenter", zap.String("datacenter", dc.Name()), zap.Error(netErr))
			}
		}
	...
	if restClient := rest.NewClient(vc.Client); restClient != nil {
		...
		if err := restClient.Login(ctx, restURL.User); err == nil {
			...
			// Content Libraries
			if libs, libErr := libManager.GetLibraries(ctx); libErr == nil {
				...
				for _, lib := range libs {
					...
					if items, itemErr := libManager.GetLibraryItems(ctx, lib.ID); itemErr == nil {
				...
				logger.Warn("failed to query content libraries", zap.Error(libErr))
			}
		} else {
			...
			if username != "" && password != "" {
				...
				if err := restClient.Login(ctx, restURL.User); err == nil {
					...
					if libs, libErr := libManager.GetLibraries(ctx); libErr == nil {
						...
						logger.Warn("failed to query content libraries after manual login", zap.Error(libErr))
					}
				} else {
